...
const VERDICT_META = {
    red: { bg: "rgba(255,78,106,0.08)", border: "rgba(255,78,106,0.25)", color: "#FF4E6A", icon: "⛔", glow: "rgba(255,78,106,0.15)" },
    yellow: { bg: "rgba(255,184,0,0.08)", border: "rgba(255,184,0,0.25)", color: "#FFB800", icon: "⚠️", glow: "rgba(255,184,0,0.12)" },
    green: { bg: "rgba(0,232,122,0.08)", border: "rgba(0,232,122,0.25)", color: "#00E87A", icon: "✅", glow: "rgba(0,232,122,0.12)" },
};
const CMCE_COLORS = {
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4 mb-8 animate-slide-up">
                    ...
                    <div className="flex gap-3 flex-wrap">
                        ...
                        >
                            {reportSaved ? "✅ Saved!" : "📄 Export Report"}
                        </button>
                        <Link href="/analyze">
                            <button className="btn-outline text-sm py-2 px-4">🔄 Analyze Another</button>
                        </Link>
                    </div>
                </div>
                ...
                {result.file_type === "whatsapp" && result.caption_verdict && (
                    ...
                    >
                        <div className="text-xs font-black mb-2 uppercase tracking-wider" style={{ color: "#FF4E6A" }}>
                            📱 Caption Claim Verification
                        </div>
                        <div className="text-sm mb-2" style={{ color: "#C5CDE8" }}>
                            Claim: <em>&quot;{result.caption_claim}&quot;</em>
                        </div>
                        <div className="font-bold text-sm" style={{ color: "#FF4E6A" }}>❌ {result.caption_verdict}</div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex flex-col gap-5 animate-slide-up" style={{ animationDelay: "0.1s", animationFillMode: "both" }}>
                        ...
                        <div className="card p-6 rounded-2xl flex flex-col items-center">
                            <div className="flex items-center justify-between w-full mb-4">
                                <h3 className="font-bold text-sm" style={{ color: "#E8EEFF" }}>🎯 Confidence Score</h3>
                        ...
                        {subScoreEntries.length > 0 && (
                            <div className="card p-5 rounded-2xl">
                                <h3 className="font-bold text-sm mb-5" style={{ color: "#E8EEFF" }}>📊 Per-Modality Scores</h3>
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-5 animate-slide-up" style={{ animationDelay: "0.15s", animationFillMode: "both" }}>
                        ...
                        {/* CMCE detail */}
                        <div className="card p-5 rounded-2xl">
                            <h3 className="font-bold text-sm mb-3" style={{ color: "#E8EEFF" }}>
                ...
                >
                    {[
                        { href: "/verify", icon: "🔏", title: "Get Authenticity Certificate", sub: "Prove your content is real", color: "#00E87A" },
                        { href: "/analyze", icon: "🔄", title: "Analyze Another File", sub: "Upload a new file", color: "#00FFD1" },
                        { href: "/api-docs", icon: "🔌", title: "Integrate via API", sub: "TrustScore API docs", color: "#8B5CF6" },
                    ].map((item) => (
